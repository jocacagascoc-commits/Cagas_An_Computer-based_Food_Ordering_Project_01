...
def show_menu():
    ...
    for code, (name, price) in FOODS.items():
        print(f" {code}. {name:<15} â‚±{price:.2f}")
...
def show_order(order):
    ...
    for i, item in enumerate(order, 1):
        print(
            f"{i}. {item['qty']} x {item['name']} = â‚±{item['price'] * item['qty']:.2f}")
    print("---------------------")
...
def payment_flow(total):
    ...
    while True:
        ...
        try:
            ...
            continue
        if cash < total:
            print(f"Not enough money. You still need â‚±{(total - cash):.2f}")
            continue
...
def print_receipt(order, mode, total, paid, change):
    ...
    for item in order:
        print(
            f"    {item['qty']} x {item['name']:<15} â‚±{item['price'] * item['qty']:.2f}")
    print("-" * 35)
    print(f"      TOTAL:          â‚±{total:.2f}")
    print(f"      CASH:           â‚±{paid:.2f}")
